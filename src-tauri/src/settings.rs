...
fn default_stt_models() -> HashMap<String, String> {
    let mut map = HashMap::new();
    for provider in default_stt_providers() {
        map.insert(provider.id.clone(), provider.default_model.clone());
    }
    map
}
...
fn ensure_stt_defaults(settings: &mut AppSettings) -> bool {
    ...
    for provider in default_stt_providers() {
        ...
        if !settings.stt_models.contains_key(&provider.id) {
            settings
                .stt_models
...
impl AppSettings {
    ...
    pub fn active_stt_provider(&self) -> Option<&SttProvider> {
        self.stt_providers
            .iter()
...
pub fn load_or_create_app_settings(app: &AppHandle) -> AppSettings {
    ...
    changed |= ensure_stt_defaults(&mut settings);
    if changed {
        store.set("settings", serde_json::to_value(&settings).unwrap());
    }
