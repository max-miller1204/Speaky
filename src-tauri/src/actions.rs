...
use crate::settings::{get_settings, AppSettings, TranscriptionMode, APPLE_INTELLIGENCE_PROVIDER_ID};
use crate::shortcut;
...
impl ShortcutAction for TranscribeAction {
    fn start(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        // Load model in the background (only for local transcription)
    ...
    fn stop(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Some(samples) = rm.stop_recording(&binding_id) {
                ...
                let transcription_result = match settings.transcription_mode {
                    TranscriptionMode::Online => {
                        let provider = settings.active_stt_provider().cloned();
                        match provider {
                            Some(provider) => {
                                let api_key = settings
                                    .stt_api_keys
                                ...
                                let result = crate::stt_client::transcribe_online(
                                    &provider,
                                    &api_key,
                                    &model,
                                    &samples,
                                ...
                                .await;
                                // Apply word correction and filler filtering for online results
                                result.map(|text| {
                                    let text = if !settings.custom_words.is_empty() {
                                        crate::audio_toolkit::text::apply_custom_words(
                                            &text,
                                    ...
                                    crate::audio_toolkit::text::filter_transcription_output(&text)
                                })
                                .map_err(|e| anyhow::anyhow!(e))
                            }
                            None => Err(anyhow::anyhow!("No STT provider configured")),
                        }
                    }
                    TranscriptionMode::Local => tm.transcribe(samples),
                };
