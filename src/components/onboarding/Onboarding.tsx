...
import SpeakyTextLogo from "../icons/SpeakyTextLogo";
...
const Onboarding: React.FC<OnboardingProps> = ({ onModelSelected }) => {
  ...
  const handleUseOnlineApi = async () => {
    await updateSetting("transcription_mode", "online" as any);
    onModelSelected();
  };
    ...
    <div className="h-screen w-screen flex flex-col p-6 gap-4 inset-0">
      ...
      <div className="max-w-[600px] w-full mx-auto text-center flex-1 flex flex-col min-h-0">
        ...
        <div className="flex flex-col items-center gap-2 pt-2 pb-4 shrink-0">
          <p className="text-text/50 text-sm">{t("onboarding.orOnlineApi")}</p>
          <button
            onClick={handleUseOnlineApi}
            disabled={downloading}
            className="px-6 py-2 rounded-lg border border-mid-gray/30 text-sm font-medium text-text/80 hover:bg-mid-gray/10 transition-colors disabled:opacity-50"
          >
            {t("onboarding.useOnlineApi")}
          </button>
        </div>
      </div>
    </div>
