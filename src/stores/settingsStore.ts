...
const settingUpdaters: {
  ...
  transcription_mode: (value) =>
    commands.changeTranscriptionMode(value as any),
};
...
export const useSettingsStore = create<SettingsStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    updateSttApiKey: async (providerId, apiKey) => {
      ...
      setUpdating(updateKey, true);
      try {
        await commands.changeSttApiKey(providerId, apiKey);
        await refreshSettings();
      } catch (error) {
    ...
    updateSttModel: async (providerId, model) => {
      ...
      setUpdating(updateKey, true);
      try {
        await commands.changeSttModel(providerId, model);
        await refreshSettings();
      } catch (error) {
    ...
    updateSttBaseUrl: async (providerId, baseUrl) => {
      ...
      setUpdating(updateKey, true);
      try {
        await commands.changeSttBaseUrl(providerId, baseUrl);
        await refreshSettings();
      } catch (error) {
